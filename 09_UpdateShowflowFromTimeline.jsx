// 09_UpdateShowflowFromTimeline - 실제 기능 구현 시작
alert("09 스크립트 시작");
...
alert("마커 개수: " + markers.numMarkers);

// 첫 번째 마커 읽기 시도
if (markers.numMarkers > 0) {
    var firstMarker = markers.getFirstMarker();
    if (firstMarker) {
        alert("첫 마커: " + (firstMarker.name || "이름없음"));
    } else {
        alert("첫 마커 읽기 실패");
    }
} else {
...
// 다음 단계: JSON 파일 선택
alert("JSON 파일 선택 다이얼로그 열기");

var jsonFile = null;
try {
    jsonFile = File.openDialog("Select showflow JSON file", "JSON:*.json");
    if (jsonFile) {
        alert("선택된 JSON 파일: " + jsonFile.fsName);
    } else {
        alert("JSON 파일 선택 취소");
    }
} catch (e) {
    alert("파일 선택 다이얼로그 실패: " + e);
}
...
if (jsonFile) {
    ...
    try {
        ...
        if (jsonFile.open("r")) {
            ...
            var jsonData = null;
            alert("jsonData 변수 초기화 완료");

            try {
                alert("JSON.parse 사용 가능 여부 체크");
                if (typeof JSON !== "undefined" && JSON.parse) {
                    alert("JSON.parse 사용");
                    jsonData = JSON.parse(jsonText);
                    alert("JSON.parse 성공");
                } else {
                    alert("JSON.parse 없음 - eval 사용");
                    jsonData = eval("(" + jsonText + ")");
                    alert("eval 성공");
                }

                alert("파싱된 데이터 타입: " + typeof jsonData);
                alert("JSON 파싱 성공\n슬롯 개수: " + (jsonData && jsonData.slots ? jsonData.slots.length : "알 수 없음"));
            } catch (parseError) {
